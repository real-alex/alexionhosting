'use strict';
/**
 * Alexion dev simulator.
 *
 * A stand-in for a real SA-MP / CRMP server so the hosting platform can be
 * developed and tested end-to-end without the proprietary game binaries.
 * It binds the game UDP port, prints a realistic console, and shuts down
 * cleanly on SIGTERM/SIGINT (so "stop"/"restart" behave correctly).
 */
...
const type = (process.env.SERVER_TYPE || 'samp').toUpperCase();
...
function shutdown(signal) {
  ...
  // Safety net if the socket is slow to close.
